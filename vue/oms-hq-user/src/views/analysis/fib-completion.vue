...
import { fibCompletionApi } from "@/api/analysis";
...
const fetchData = async () => {
  try {
    ...
    const response = await fibCompletionApi.getData(params);
    ...
    if (response) {
      ...
      const successSum = numericSuccess.reduce((acc, cur) => acc + cur, 0);
      const completedSum = numericCompleted.reduce((acc, cur) => acc + cur, 0);
