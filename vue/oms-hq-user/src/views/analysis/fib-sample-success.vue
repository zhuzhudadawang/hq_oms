...
import { fibSampleSuccessApi,fibInternalSampleSuccessApi,fibExternalSampleSuccessApi } from "@/api/analysis";
...
const fetchData = async () => {
  try {
    ...
    if (currentType.value === 'total') {
      // 总成功 - 原接口
      response = await fibSampleSuccessApi.getData(params);
    } else if (currentType.value === 'internal') {
      // 内部成功 - 内部接口
      response = await fibInternalSampleSuccessApi.getData(params);
    } else if (currentType.value === 'external') {
      // 外部成功 - 外部接口
      response = await fibExternalSampleSuccessApi.getData(params);
    }
