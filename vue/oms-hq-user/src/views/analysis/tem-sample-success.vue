    ...
    <el-card>
      <template #header>
        <div class="card-header">
          ...
          >
            <el-option label="总成功" value="total"></el-option>
            <el-option label="内部成功" value="internal"></el-option>
            <el-option label="外部成功" value="external"></el-option>
          </el-select>
...
import { temSampleSuccessApi,temInternalSampleSuccessApi,temExternalSampleSuccessApi } from "@/api/analysis";
...
const fetchData = async () => {
  try {
    ...
    if (currentType.value === 'total') {
      // 总成功 - 原接口
      response = await temSampleSuccessApi.getData(params);
    } else if (currentType.value === 'internal') {
      // 内部成功 - 内部接口
      response = await temInternalSampleSuccessApi.getData(params);
    } else if (currentType.value === 'external') {
      // 外部成功 - 外部接口
      response = await temExternalSampleSuccessApi.getData(params);
    }
