<template>
  <div class="market-zhuhai-key-customers">
  ...
  customers: [
    {
      name: '淇澳岛（全志）',
      series: [
        { label: '2024', data: [9, 12, 15, 13, 13, 9, 10, 12, 31, 24, 18, 16] },
        { label: '2025', data: [12, 17, 18, 13, 15, 13, 12, 14, 16, 8, 6, 0] }
      ]
    }
  ]
}
...
const fetchData = async () => {
  ...
  try {
    ...
    const payload = await marketAnalysisApi.getZhuhaiKeyCustomers(params)
    console.log('珠海重要客户订单统计数据获取成功。', payload)
    const normalized = normalizePayload(payload)
    if (!normalized) throw new Error('invalid-payload')
    applyData(normalized)
  } catch (error) {
    console.warn('珠海重要客户订单统计数据获取失败，使用示例数据。', error)
  } finally {
    loading.value = false
  }
}
