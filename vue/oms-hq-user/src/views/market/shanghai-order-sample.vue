<template>
  <div class="market-order-sample">
    <el-card class="chart-card" v-loading="loading">
      <template #header>
        <div class="card-header">
          <span>上海订单 & 样点统计（日度柱状）</span>
...
const DEFAULT_DATA = {
  ...
  orders: [132, 138, 140, 146, 152, 158, 165],
  samples: [62, 65, 66, 70, 74, 76, 80]
}
...
const fetchData = async () => {
  ...
  try {
    ...
    const payload = await marketAnalysisApi.getShanghaiOrderSample(params)
    console.log('上海订单与样点统计数据获取成功。', payload)
    const normalized = normalizePayload(payload)
    if (!normalized) throw new Error('invalid-payload')
    applyData(normalized)
  } catch (error) {
    console.warn('上海订单与样点统计数据获取失败，使用示例数据。', error)
  } finally {
    loading.value = false
  }
}
