<template>
  <div class="business-fib-tem-revenue">
    <el-card class="chart-card" v-loading="loading">
      <template #header>
        <div class="card-header">
          <span>FIB/TEM检测暂估收入（月度）</span>
...
const fetchData = async () => {
  loading.value = true
  try {
    const response = await businessAnalysisApi.getFibTemEstimatedRevenue()
    const applied = applyPayload(response)
    if (!applied) throw new Error('empty-data')
  } catch (error) {
    console.warn('FIB/TEM检测暂估收入数据获取失败。', error)
  } finally {
    loading.value = false
  }
}
